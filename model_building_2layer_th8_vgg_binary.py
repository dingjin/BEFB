...
def threshold2binary(x, t=0.8):
    def _threshold2binary_grad(grad):
        ...
        return 0 * grad
        #return tf.exp(-(tf.abs(x)-thresholdsobel_extend)) / (1 + tf.exp(-(tf.abs(x)-thresholdsobel_extend))) * (1 + tf.exp(-(tf.abs(x)-thresholdsobel_extend))) * grad
        #return 1 / (1 + tf.exp(-(tf.abs(x)-thresholdsobel_extend))) * (1 - (1 / (1 + tf.exp(-(tf.abs(x)-thresholdsobel_extend))))) * grad #1 * grad
    x1 = tf.abs(x)
    ...
    #grad_cus = 10*tf.exp(-10*(tf.abs(x)-thresholdsobel_extend)) / tf.math.pow((1 + tf.exp(-10*(tf.abs(x)-thresholdsobel_extend))),2)
