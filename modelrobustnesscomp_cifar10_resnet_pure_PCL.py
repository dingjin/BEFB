def comp_cifar10_resnet_pure_PCL(numberofcommon=0, repeat=1, datetime='0601', epsilons=8, stepsize=1, steps=8, numberofcommoncw=100):
    ...
    from model_building_resnet_pure import ResNet34
    ...
    pathstring = '/mnt/ex/PycharmProjects/BinaryFeatures/PCL/weiResNetpureModel'
    pathstring += datetime
    pathstring += '-cifar10-repeat'
    pathstring += str(repeat)
    pathstring += '/my_checkpoint'
    model_binary = ResNet34()
    ...
    """
    pathstring1 = '/mnt/ex/PycharmProjects/BinaryFeatures/dataset-cifar10/weiResNetBinaryModel'
    pathstring1 += datetime
    pathstring1 += '-3layer-th8-repeat'
    pathstring1 += str(repeat)
    pathstring1 += '/my_checkpoint'

    model_binary1 = ResNet34binary()
    model_binary1.load_weights(pathstring1)
    """
    ...
    """
    with open('/mnt/ex/PycharmProjects/BinaryFeatures/dataset-cifar10/advex_resnet_pure.npy', 'wb') as f:
        np.save(f, adv_x_0)
    f.close()

    with open('/mnt/ex/PycharmProjects/BinaryFeatures/dataset-cifar10/advex_truelabel_resnet_pure.npy', 'wb') as f:
        np.save(f, y_test_binary_ndarray_common)
    f.close()
    """
    # save loss and accuracy, append mode 2023-6-4
    pathstring = '/mnt/ex/PycharmProjects/BinaryFeatures/PCL/'
    namestring = 'weiResNetpureModel'
    namestring += datetime
   ...
   # 先跑一次
    ...
    model_concat = keras.Model(inputs=model_binary.layers[0].input, outputs=model_binary.get_layer('dense').output)
...
if __name__ == '__main__':
    ...
    comp_cifar10_resnet_pure_PCL(numberofcommon=argv1, repeat=argv2, datetime=sys.argv[3], epsilons=argv4, stepsize=argv5, steps=argv6, numberofcommoncw=argv7)
