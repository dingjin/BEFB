def main_cifar_resnet_pure(epochs = 200, batchsize = 50, repeat = 1, datetime='0601'):
    ...
    model_V.call(inputs)
    model_concat1 = keras.Model(inputs=model_V.layers[0].input, outputs=model_V.get_layer('conv2d_20').output)
    model_concat2 = keras.Model(inputs=model_V.layers[0].input, outputs=model_V.get_layer('dense').output)

    @tf.function
    def train_step(x, y, output_conv_dis, output_dense_dis):
        with tf.GradientTape() as tape:
            ...
            loss_value = loss_value + output_conv_dis + output_dense_dis
    ...
    def prototypeconformity(x, y):
        ...
        return output_conv_dis, output_dense_dis
    ...
    def computing_distance_conv(inputfeatures, labels):
        ...
        while i < numberofclass:
            classcenter_temp = tf.reduce_mean(
                sort_features[index_in_sort_features:index_in_sort_features + samplesineachclass[0, i]], 0)
    ...
    for epoch in range(num_epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            output_conv_dis, output_dense_dis = prototypeconformity(x_batch_train, y_batch_train)
            loss_value = train_step(x_batch_train, y_batch_train, output_conv_dis, output_dense_dis)
...
if __name__ == '__main__':
    ...
    main_cifar_resnet_pure(epochs=argv1, batchsize=argv2, repeat=argv3, datetime=argv4)
    """
    argv1 = int(sys.argv[1])
    argv2 = int(sys.argv[2])
    argv3 = int(sys.argv[3])
    main_cifar_resnet_pure(epochs=argv1, batchsize=argv2, repeat = argv3, datetime=sys.argv[4])
